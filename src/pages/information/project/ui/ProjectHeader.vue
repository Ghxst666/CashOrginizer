...
type ProjectPeriod = 'all_period' | 'today' | 'yesterday' | 'last_month' | 'current_month' | 'next_month' | 'custom'
...
const emits = defineEmits<{
    openDialog: []
    selectPeriod: [filter: ProjectPeriodFilter]
}>()

const periodTitles: Record<ProjectPeriod, string> = {
...
const selectedPeriodTitle = computed(() => {
    ...
    return periodTitles[props.selectedPeriod as ProjectPeriod] ?? 'Период'
})
...
function handlePeriodCommand(period: ProjectPeriod) {
    if (period === 'custom') {
